...
const withInfoPlistXcodeDevelopmentTeam = (config, id) => {
  return withInfoPlist(config, async(config) => {
    ...
    let PodfileOriginalContents = fs.readFileSync('ios/enhmessageme.xcodeproj/project.pbxproj', {encoding: 'utf8'});
    ...
    if ( ! PodfileOriginalContents.includes('DEVELOPMENT_TEAM')) {
      ...
      fs.writeFileSync('ios/enhmessageme.xcodeproj/project.pbxproj', PodfileModifiedContents, {encoding: 'utf8'});
