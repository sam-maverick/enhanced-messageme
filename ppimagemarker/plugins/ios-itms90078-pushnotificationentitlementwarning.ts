...
const withInfoPlistPushNotificationEntitlement = (config, id) => {
  return withInfoPlist(config, async(config) => {
    ...
    let AppDelegateOriginalContents = fs.readFileSync('ios/ppimagemarker/AppDelegate.mm', {encoding: 'utf8'});
...
'
    ...
    fs.writeFileSync('ios/ppimagemarker/AppDelegate.mm', AppDelegateModifiedContents, {encoding: 'utf8'});
